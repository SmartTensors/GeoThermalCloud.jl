# MULTIPROCESSING ON MacOSX -- Train 21875 models
#
# mpirun -n 1 python get_dnn_results_nodataug.py >> get_dnn_results_nodataug.txt (if using mpi4py)
# python get_dnn_results_nodataug.py (when using multiprocessing)
#
...
#
# mpirun -n 50 get_dnn_results_nodataug.py >> get_dnn_results_nodataug.txt #PNNL's pinklady
# srun -n 32 -c 2 --cpu-bind=cores python get_dnn_results_nodataug.py >> hp_output_ss_nersc.txt #Haswell
# srun -n 68 -c 4 --cpu-bind=cores python get_dnn_results_nodataug.py >> hp_output_ss_nersc.txt #KNL
# killall python
#
...
#=============================================================================;
#  Function-4: Train individual models (multiprocessing calls this function)  ; 
#=============================================================================;
def get_trained_models(args_inp_list):

	#---------------------------------;
	#  0. Unzip input arguments list  ;
	#---------------------------------;
	random_seed, counter = args_inp_list
	...
	#
	path_fl_sav    =  path_testing + "1_InvDNNModel_ss_mac/" + str(counter) + "_model/" #i-th hp-dl-model folder
	...
	#-----------------------------------;
	#  11. Print R2-score on test data  ;
	#-----------------------------------;
	r2 = r2_score(test_pred_q.flatten(), test_q.flatten()) #r2-score
	print('R2-score = ', r2)
...
#**************************************************;
num_procs        = 1 #Total number of processors
#
start            = 21875
end              = 21876 #21876
...
if __name__ == '__main__':
	...
	#=======================================================;
	#  1. Create args and pack args for DNN model training  ;
	#=======================================================;
	random_seed_list = [131 for i in range(0,len(id_list))] #Popular random seeds
	#
	args_inp_list    = list(zip(random_seed_list,id_list)) #Args list for embarassingly parallel function -- get_trained_models

	#================================================;
	#  2. Create processor pool and save DNN models  ;
	#================================================;
	pool      = multiprocessing.Pool(processes = num_procs) #Create a pool of workers
	results   = pool.map(get_trained_models, args_inp_list, chunksize = 1) #Save keras-tuner searches
	print(results)
	pool.close()
	pool.join()
	pool.terminate()
