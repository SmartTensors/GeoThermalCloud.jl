...
attributes_short = ["wellid", "md", "azimuth", "inclination", "x", "y", "z", "casing", "fluids", "use", "production", "use2", "normal", "coulomb", "dilation", "faults", "td", "ts", "curve", "modeltemp", "faultdense", "faultintdense", "distfromcontacts", "distfromfaults", "unitthickness", "goodlith", "confidence"]
ai = indexin(attributes_process, attributes_short)

d, h = DelimitedFiles.readdlm("data/AllBradyWells_LANL_ML.txt", ','; header=true)
...
for i=ai; @info attributes_col[i], i; display(unique(sort(d[:,i]))); end
